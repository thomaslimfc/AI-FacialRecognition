...
# Define constants
IMG_HEIGHT, IMG_WIDTH = 224, 224  # Adjust if EfficientNetV2 requires different size
BATCH_SIZE = 32

# Helper function to extract labels from filenameskn
...
model_path = 'efficientnetv2_gender_age_model.h5'  # Replace with your EfficientNetV2 model path
loaded_model = tf.keras.models.load_model(model_path)

# Make predictions on the test dataset
predictions = loaded_model.predict(test_dataset)

# Check if predictions are returned as a list (e.g., [age_predictions, gender_predictions])
if isinstance(predictions, list) and len(predictions) == 2:
    predicted_age = predictions[0]   # Age predictions
    predicted_gender = predictions[1]  # Gender predictions
else:
    raise ValueError("Unexpected prediction format from the model. Expected a list with two elements.")
